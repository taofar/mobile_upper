<template>
	<view class="content">
		...
		<!-- <uni-row class="device-link-row">
			<view class="device-link-col"></view>
		</uni-row> -->
	</view>
</template>
	...
	export default {
		...
		beforeMount() {
			...
			this.timer = setInterval(()=>{
				...
				let link = this.linkInfo;
				uni.getBLEDeviceRSSI({
					deviceId,
					success(res) {
						//this.$set(this.linkInfo.dbm, res.RSSI);
						// console.log(res.RSSI);
						link.dbm = res.RSSI;
					},
					fail(err) {
						if (err.errCode != 0) {
							this.$emit('onBluetoothLosted', {link:false});
		...
		methods: {
			...
			getBluetoothDevices() {
				uni.getBluetoothDevices({
					success: res => {
						...
						let prefix = this.bleNamePrefix;
						console.log(prefix);
						for (let item of res.devices) {
							//if (item.name && item.name.length != 0) console.log(JSON.stringify(item));
							//if (item.name.length > 4 && (item.name.match(/^1KM-/) || item.name.match(/^SWIFT-/))) {
							if (item.name.length > 4 && (item.name.match(prefix))) {
			...
			getBLEDeviceServices() {
				...
				uni.getBLEDeviceServices({
					deviceId,
					success: res => {
						this.linkInfo.service = [];
						if (res.services.length == 0) {
							console.log("get services is null, get again.");
							if (this.bluetoothLinking) {
								setTimeout(()=>{
									this.getBLEDeviceServices();
								}, 200);
						...
						for (let item of res.services) {
							this.linkInfo.service.push(item.uuid);
						}
						console.log(this.linkInfo.service);
					...
					fail: err => {
						console.log('获取服务失败，错误码：' + err.errCode);
						if (this.bluetoothLinking) {
							setTimeout(()=>{
								this.getBLEDeviceServices();
							}, 200);
						...
						if (err.errCode !== 0) {
							bluetoothLinkFailTip(err.errCode);
						}
					}
				})
			},
			...
			getBLEDeviceCharacteristics() {
				...
				for (let service of this.linkInfo.service) {
					console.log(service);
					if (service.substring(4, 8) == "FFE0") serviceId = service;
				}
				this.linkInfo.service = [];
				this.linkInfo.service.push(serviceId);
				...
				uni.getBLEDeviceCharacteristics({
					...
					success: res => {
						...
						let flag = false;
						for (let item of res.characteristics.reverse()) {
							if (flag == false && item.properties.notify) {
								console.log(item);
						...
						this.setBLEMTU();
						this.$emit('onBluetoothLinked', {linked: true, send: this.bluetoothSendString});
					},
					fail: e => {
